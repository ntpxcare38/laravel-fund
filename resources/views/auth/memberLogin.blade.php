@extends('layoutLogin')
@section('title','เข้าสู่ระบบสำหรับ (สมาชิก)')
@section('content')

                    <form method="POST" action="{{ route('member-login') }}" class="frmLogin">
                            <H4>เข้าสู่ระบบสำหรับ (สมาชิก)</H4>
                        @csrf

                        <div class="row">
                            <div class="input-field col s11 m10">
                                <i class="material-icons prefix">perm_identity</i>
                                <input id="mem_card_id" type="text" class="form-control{{ $errors->has('mem_card_id') ? ' is-invalid' : '' }}" onkeyup='javascript:controlchars(this,number)' onkeydown='javascript:controlchars(this,number)' name="mem_card_id" value="{{ old('mem_card_id') }}" maxlength="13" required autofocus>
                                    <label for="icon_prefix">เลขประจำตัวประชาชน</label>
                                    @if ($errors->has('mem_card_id'))
                                        <span><font color="red">
                                            <strong>{{ $errors->first('mem_card_id') }}</strong>
                                        </font></span>
                                    @endif
                            </div>
                        </div>
                        ...
                        <div class="row center">
                            ...
                            <div class="input-field col s12 m12">
                                <p align="left">
                                    <label>วิธีเข้าใช้งาน</label><br>
                                    <label>1. กรอกเลขประจำตัวประชาชน 13 หลัก</label><br>
                                    <label>2. รหัสผ่านเริ่มต้นใช้งานเป็น วันเดือนปีเกิด เช่น 01012562</label><br>
                                    <label>3. หากมีปัญหาการเข้าใช้งานระบบให้ติดต่อเจ้าหน้าที่ตามข้อมูลติดต่อ</label>
                                </p>
                                <p align="left">
                                    <label>**หมายเหตุการเข้าใช้งานระบบ : ผู้ใช้ต้องเป็นสมาชิก{{$fund->fund_name}} อำเภอ{{$fund->fund_district}} จังหวัด{{$fund->fund_province}}</label>
                                </p>
                            </div>
                        </div>
                    </form>
